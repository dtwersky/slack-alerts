...
class TestSafeFmt(unittest.TestCase):
    ...
    def test_nested_object_missing(t):
        ...
        t.assertEqual(
            safe_format("{foo[0].ding}", dict(foo=[dict(x="DONG")])), "{foo[0].ding}"
        )
        t.assertEqual(safe_format("{foo[0].ding}", dict(foo=[])), "{foo[0].ding}")
        t.assertEqual(safe_format("{foo[0].ding}", dict()), "{foo[0].ding}")

    def test_no_attr_access(t):
        t.assertEqual(safe_format("{foo.__class__}", dict(foo="1")), "{foo.__class__}")
